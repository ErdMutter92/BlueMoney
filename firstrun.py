...
'''
from urllib2 import *
import os, base64, hashlib, sys, textwrap, repair
...
class firstrun():
    ...
    def mkCSDV(self, title, content):
        if not os.path.exists('./data/'+title+'.csdv'):
            print('Creating blank CSDV File for '+title+' items...')
            file = open('./data/'+title+'.csdv', 'a')
            file.write(content)
            file.close()
            return 1
        else:
    ...
    def genFile(self, title, ext, location = 'data/resources/'):
